...
//Rare exceptions to indicate a bug in the _tinkerforge code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_tinkerforge = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_tinkerforge]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.tinkerforge.module"
